...
def build_app_type_map(vehicle):
    """遍历整车 json 中所有回路，建立 用电器名称 -> 用电器类型 的映射。"""
    app_map = {}
    arr = _get_circuit_list(vehicle) if isinstance(vehicle, dict) else vehicle
    for c in arr:
        a = c.get('起点用电器名称')
...
def build_appliance_chain_rows(circuit, app_type_map):
    """读取后端输出的『能量流用电器路径』，反转为 源->消费端 方向，
    生成用电器链的每行字段：
        [序号, 用电器名称, 用电器类型, 该用电器类型(中文说明)]
    只输出绕路的能量流用电器链。
    """
    rows = []
    if not isinstance(circuit, dict):
        return rows
    chain = circuit.get('能量流用电器路径')
    if not chain:
        return rows
    # 后端 appliancePath 是 消费端(锚点)->源端 方向，反转成 源->消费端
    chain = list(reversed(chain))
    for idx, app in enumerate(chain, 1):
        app = (app or '').strip()
        # 合点/位置点用 [] 包裹
        if app.startswith('[') and app.endswith(']'):
            ...
            app_type = app_type_map.get(app, '') or '未知类型'
            display = app
        rows.append([idx, display, app_type])
    return rows
...
# --------------------------------------------------------------------------- #
# Excel 导出：回路自身 + 能量流绕路/不绕路走向
# --------------------------------------------------------------------------- #
def _circuit_pos_pair(c):
    """返回回路的 (起点位置, 终点位置)（终点为 焊点位置名称 兜底），用于按位置配对"""
    s = c.get('起点位置名称')
    e = c.get('终点位置名称') or c.get('焊点位置名称')
    return s, e


def find_circuit_by_pos_pair(vehicle, pos_a, pos_b):
    """在整车所有回路里，找 起点/终点位置 恰好为 (pos_a,pos_b) 或 (pos_b,pos_a) 的回路。
    返回该回路 dict，找不到返回 None。"""
    if not pos_a or not pos_b or pos_a == pos_b:
        return None
    arr = _get_circuit_list(vehicle) if isinstance(vehicle, dict) else vehicle
    for c in arr:
        s, e = _circuit_pos_pair(c)
        if (s == pos_a and e == pos_b) or (s == pos_b and e == pos_a):
            return c
    return None


def build_branch_len_map(edges):
    """从拓扑 edges 构建 位置对 -> 分支长度(米) 的映射。

    用 startPointName/endPointName 作为位置对，优先取 length，其次 referenceLength。
    长度字段单位为 mm，这里转成米并保留2位小数。
    """
    m = {}
    for e in edges:
        a = e.get('startPointName')
        b = e.get('endPointName')
        if not a or not b:
            continue
        raw = e.get('length')
        if raw is None or raw == '':
            raw = e.get('referenceLength')
        try:
            mm = float(raw)
        except (TypeError, ValueError):
            mm = 0.0
...
def circuit_branch_detail(c, edges, branch_len_map):
    """返回某根回路的 途径分支点 + 每根分支长度 的明细行列表。

    每根回路的 回路途径分支点 是位置名序列，相邻两点之间是一根分支。
    在 edges/branch_len_map 里查每根分支长度。
    若回路没有途径分支点（或不足2个点），则退化为把 起点位置->终点位置 作为一根分支。
    返回形如 [ [位置A, 位置B, 长度m], ... ]；无法匹配长度的填空。
    """
    pts = c.get('回路途径分支点') or []
    if len(pts) < 2:
        # 退化为 起点->终点 单根分支（回路本身就占一根分支）
        a = c.get('起点位置名称')
        b = c.get('终点位置名称') or c.get('焊点位置名称')
        if not a or not b:
            return []
        length = branch_len_map.get(a + '|' + b, '')
        return [[a, b, length]]
    details = []
    for i in range(len(pts) - 1):
        ...
        if a == b:
            continue
        length = branch_len_map.get(a + '|' + b, '')
        details.append([a, b, length])
    return details


def build_no_detour_branch_rows(circuit, edges):
    """返回能量流【不绕路】的途径分支点 + 分支长度 的明细行列表。

    数据源：回路的『能量流不绕路途径分支id』(分支id列表)。
    在 edges 里按分支id反查 起点名称/终点名称/长度。
    返回 [ [位置A, 位置B, 长度m], ... ]；无法匹配长度的填空。
    """
    rows = []
    if not isinstance(circuit, dict):
        return rows
    branch_ids = circuit.get('能量流不绕路途径分支id') or []
    if not branch_ids:
        return rows
    # 分支id -> (起点, 终点, 长度m)
    id_info = {}
    for e in edges or []:
        ...
        try:
            ...
            mm = 0.0
        meters = round(mm / 1000.0, 2)
        id_info[bid] = (a, b, meters)
    for bid in branch_ids:
        info = id_info.get(bid)
        if info:
            a, b, meters = info
            rows.append([a, b, meters])
        else:
            rows.append([bid, '', ''])
    return rows


def build_energy_flow_rows(vehicle, circuit, pos_app_map,
                           path_field='能量流回路id路径', edges=None):
    """以「回路」为单位输出能量流走向，每根回路下方附带其途径分支点+分支长度。

    直接读取后端输出到该回路的 回路id链（path_field，如 '能量流回路id路径'），
    按能量流走向顺序逐条输出回路信息。
    返回行列表，每根回路包含：
        主行: [序号, 回路编号, 起点位置, 起点用电器, 起点用电器类型,
              终点位置, 终点用电器, 终点用电器类型]
        其下若 edges 提供且该回路有途径分支点，追加若干分支明细行:
              [None, '  分支:位置A→位置B', '长度Xm', '', ...]  （首列 None 表示缩进明细）
    """
    rows = []
    circuit_path = circuit.get(path_field) if isinstance(circuit, dict) else None
    if not circuit_path:
        return rows

    # 后端存储的 circuitPath 是 消费端(锚点回路) -> 源端 的顺序(DFS 回溯方向)，
    # 展示上需要反转成 源端 -> 消费端(锚点回路) 的方向。
    circuit_path = list(reversed(circuit_path))
    ...
    for c in arr:
        cid = c.get('回路id')
        if cid:
            id_map.setdefault(cid, c)

    # 位置对 -> 分支长度
    branch_len_map = build_branch_len_map(edges) if edges else {}

    for idx, cid in enumerate(circuit_path, 1):
        c = id_map.get(cid)
        if c is None:
            rows.append([idx, cid, '', '', '', '', '', ''])
            continue
        rows.append([
            ...
            c.get('起点用电器名称'),
            c.get('起点用电器类型'),
            c.get('终点位置名称') or c.get('焊点位置名称'),
            c.get('终点用电器名称'),
            c.get('终点用电器类型'),
            ])
        # 该回路途径分支明细（若 edges 提供）
        if branch_len_map:
            details = circuit_branch_detail(c, edges, branch_len_map)
            for a, b, length in details:
                rows.append([None, f"  分支: {a} → {b}", length, '', '', '', '', ''])
    return rows


def export_excel(save_path, circuit, detour_rows, no_detour_rows):
    """把 回路自身 + 能量流绕路 + 能量流不绕路(途径分支点+长度) 导出为 Excel。
    """
    ...
    ws1.append(['字段', '值'])
    fields = [
        ('回路编号', circuit.get('回路编号')),
        ('回路信号名', circuit.get('回路信号名')),
        ('起点用电器', circuit.get('起点用电器名称')),
        ('起点用电器类型', circuit.get('起点用电器类型')),
        ('起点位置', circuit.get('起点位置名称')),
        ('终点用电器', circuit.get('终点用电器名称')),
        ('终点用电器类型', circuit.get('终点用电器类型')),
        ('终点位置', circuit.get('终点位置名称') or circuit.get('焊点位置名称')),
        ('能量流绕路长度(米)', circuit.get('能量流绕路长度总值(米)')),
        ('能量流不绕路长度(米)', circuit.get('能量流不绕路长度(米)')),
    ]
    for k, v in fields:
        ws1.append([k, v])
    ...
    ws1.append([])
    ws1.append(['途径分支点顺序', '', ''])
    br_pts = circuit.get('回路途径分支点') or []
    for i, p in enumerate(br_pts, 1):
        ws1.append([i, p, ''])
    _style_header(ws1)

    # ---------- Sheet2: 能量流绕路(按回路链，源->消费端) ----------
    ws2 = wb.create_sheet('能量流绕路')
    _write_path_sheet(ws2, detour_rows, '能量流绕路走向(按回路, 源头->消费端)')

    # ---------- Sheet3: 能量流不绕路(途径分支点+分支长度) ----------
    ws3 = wb.create_sheet('能量流不绕路')
    ws3.append(['能量流不绕路走向(途径分支点 + 分支长度)'])
    ws3.append([])
    ws3.append(['序号', '分支起点', '分支终点', '分支长度(米)'])
    for idx, r in enumerate(no_detour_rows, 1):
        a, b, meters = r
        ws3.append([idx, a, b, meters])
    _style_header(ws3, header_row=3)

    wb.save(save_path)
    print(f"[OK] Excel 已保存: {save_path}")


def _write_path_sheet(ws, rows, title):
    ws.append([title])
    ws.append([])
    ws.append(['序号', '回路编号', '起点位置', '起点用电器', '起点用电器类型',
               '终点位置', '终点用电器', '终点用电器类型'])
    for r in rows:
        ws.append(r)
...
def main():
    ...
    parser.add_argument('--circuit-number', type=str, default=149724,
    ...
    parser.add_argument('--excel', default='energy_flow.xlsx',
                        help='输出 Excel 路径(含回路自身/能量流绕路/不绕路走向)，默认 energy_flow.xlsx')
    ...
    # 生成 Excel：回路自身 + 能量流绕路(按回路链) + 能量流不绕路(途径分支点+长度)
    detour_rows = build_energy_flow_rows(vehicle, circuit, pos_app_map,
                                         '能量流回路id路径', edges)
    no_detour_rows = build_no_detour_branch_rows(circuit, edges)
    export_excel(args.excel, circuit, detour_rows, no_detour_rows)
