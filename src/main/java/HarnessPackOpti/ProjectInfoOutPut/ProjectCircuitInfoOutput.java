...
public class ProjectCircuitInfoOutput {
    ...
    public static void main(String[] args) throws Exception {
        File file = new File("F:\\office\\idearProjects\\project20251009\\src\\main\\resources\\BS4EM初始json包含优化设置.txt");
    ...
    public String projectCircuitInfoOutput(String fileStringFormat) throws Exception {
        ...
        for (Map<String, Object> loopInfo : loopInfos) {
            if (loopInfo.get("回路起点用电器").toString().startsWith("[")
                    || loopInfo.get("回路终点用电器").toString().startsWith("[")) {
                if (loopInfo.get("回路起点用电器").toString().startsWith("[")) {
        ...
        Map<String, List<Map<String, String>>> fixMultiLoopInfos = (Map<String, List<Map<String, String>>>) fixedLoopInfosMap
                .get("multiLoopInfos");
        ...
        for (String name : set) {
            ...
            Map<String, Object> groupInfo = findGroupInfo(name, fixMultiLoopInfos.get(name), adjacencyMatrixGraph,
                    elecFixedLocationLibrary, projectInfo, true, null, electricalSet, elecBusinessPrice);
        ...
        for (List<Map<String, Object>> grouplist : grouplists) {
            ...
            List<Map<String, String>> nonfixTwoPoints = (List<Map<String, String>>) nonfixedLoopInfosMap
                    .get("twoPoint");
            Map<String, List<Map<String, String>>> nonfixMultiLoopInfos = (Map<String, List<Map<String, String>>>) nonfixedLoopInfosMap
                    .get("multiLoopInfos");
            ...
            for (Map<String, Object> objectMap : allpossibility) {
                ...
                for (Map<String, String> list : nonfixTwoPoints) {
                    // 处理两点直连回路
                    Map<String, Object> twoPointInfo = findTwoPointInfo(list, projectInfo, adjacencyMatrixGraph,
                            elecFixedLocationLibrary, false, objectMap, electricalSet, elecBusinessPrice);
                ...
                for (String name : nonfixMultiLoopInfosSet) {
                    // 处理焊点族群回路
                    Map<String, Object> groupInfo = findGroupInfo(name, nonfixMultiLoopInfos.get(name),
                            adjacencyMatrixGraph, elecFixedLocationLibrary, projectInfo, false, objectMap,
                            electricalSet, elecBusinessPrice);
                    if (groupInfo == null) {
                        flag = false;
                        break;
                    }
        ...
        List<Map<String, String>> nonfixedNotGroupLoopsTwo = (List<Map<String, String>>) nonfixedLoopInfosMap
                .get("twoPoint");
        Map<String, List<Map<String, String>>> nonfixedNotGroupLoopsMapMultiLoopInfos = (Map<String, List<Map<String, String>>>) nonfixedLoopInfosMap
                .get("multiLoopInfos");
        ...
        for (Map<String, String> list : nonfixedNotGroupLoopsTwo) {
            Map<String, Object> twoPointInfo = findTwoPointInfo(list, projectInfo, adjacencyMatrixGraph,
                    elecFixedLocationLibrary, false, bestInterFaceInfo, electricalSet, elecBusinessPrice);
            loopdetails.put(twoPointInfo.get("回路id").toString(), twoPointInfo);
        }
        // 对焊点的进行一个计算 并将最优的结果添加到loopdetails里面
        // 对焊点的进行计算
        ...
        for (String name : nonfixMultiLoopInfosSet) {
            Map<String, Object> groupInfo = findGroupInfo(name, nonfixedNotGroupLoopsMapMultiLoopInfos.get(name),
                    adjacencyMatrixGraph, elecFixedLocationLibrary, projectInfo, false, bestInterFaceInfo,
                    electricalSet, elecBusinessPrice);
            if (groupInfo == null) {
                continue;
            }
        ...
        for (Map<String, Object> loopInfo : loopInfos) {
            ...
            if (!startApp.isEmpty()) {
                ...
                if (!appMap.containsKey(startApp)) {
                    ...
                    if (!startAppPort.isEmpty()) {
                        // 找到用电器接口对应的map集合
                        Map<String, Set<String>> interfaceMap = (Map<String, Set<String>>) appMap.get(startApp)
                                .get("interfaceMap");
            ...
            if (!endApp.isEmpty()) {
                if (!appMap.containsKey(endApp)) {
                    ...
                    if (!endAppPort.isEmpty()) {
                        Map<String, Set<String>> interfaceMap = (Map<String, Set<String>>) appMap.get(endApp)
                                .get("interfaceMap");
        ...
        for (String s : bundleIdList) {
            ...
            for (Map<String, String> stringStringMap : edges) {
                if (s.equals(stringStringMap.get("分支id编号"))) {
                    ...
                    String startPoint = getWaterParam(startPointName,
                            (List<Map<String, String>>) projectInfo.get("所有端点信息"));
                    String endPoint = getWaterParam(endPointNameString,
                            (List<Map<String, String>>) projectInfo.get("所有端点信息"));
            ...
            Map<String, String> colorMap = getColorByEdges(list, circuitInfo, wet.equals("W"),
                    elecFixedLocationLibrary);
        ...
        Map<String, Map<String, String>> connectedEdgesByMatrix = findConnectedEdgesByMatrix(points,
                bundeleRelatedCircuitInfo, adjacencyMatrixGraph, edges, loopdetails);
    ...
    public void circuitCoilingLength(Map<String, Object> loopdetails, List<Map<String, String>> edges,
            GenerateTopoMatrixConnector adjacencyMatrixGraphConnector, Map<String, Object> projectInfo) {
        ...
        for (String id : idSet) {
            ...
            // 根据现有邻接列表查找两点的最短路径
            List<Integer> shortestPath = shortestPathSearch.findShortestPathBetweenTwoPoint(
                    adjacencyMatrixGraphConnector.getAdj(),
    ...
    public Map<String, Object> group(Map<String, Object> projectInfo,
        ...
        for (String s : circuitProjectInfo.keySet()) {
            ...
            for (Map<String, Object> map : circuitProjectInfoList) {
                ...
                List<Integer> shortestPathBetweenTwoPoint = findShortestPath.findShortestPathBetweenTwoPoint(
                        adjacencyMatrixGraph.getAdj(),
                        adjacencyMatrixGraph.getAllPoint().indexOf(findNode(map.get("回路起点用电器").toString(), app)),
                        adjacencyMatrixGraph.getAllPoint().indexOf(findNode(map.get("回路终点用电器").toString(), app)));
                // 获取最短路径每个点的用电器名称
                List<String> listname = convertPathToNumbers(shortestPathBetweenTwoPoint,
    ...
    public static Map<String, Object> circuitInfoByEdge(String edgeId, Map<String, Object> pointList, String edgeName) {
        ...
        for (String s : stringSet) {
            ...
            if (list.contains(edgeId)) {
                wirePriceMap.put(objectMap.get("导线选型").toString(),
                        Double.parseDouble(objectMap.get("导线单价").toString()));
                ...
                totalCost.put("回路总重量",
                        Double.parseDouble(df.format(Double.parseDouble(totalCost.get("回路总重量").toString())
                                + Double.parseDouble(objectMap.get("回路重量").toString()))));
                totalCost.put("回路总长度",
                        Double.parseDouble(df.format(Double.parseDouble(totalCost.get("回路总长度").toString())
                                + Double.parseDouble(objectMap.get("回路长度").toString()))));
                totalCost.put("端子总成本",
                        Double.parseDouble(df.format(Double.parseDouble(totalCost.get("端子总成本").toString())
        ...
        if (mapList.size() > 0) {
            avgLength = Double
                    .parseDouble(df.format(Double.parseDouble(totalCost.get("回路总长度").toString()) / mapList.size()));
        }
    ...
    /**
     * @Description 在所有的方案中找出最优的一个方案(再次遍历所有方案 ， 根据与最小成本方案的成本和重量差异计算得分 ， 最小成本方案得分为 -
     *              4.0 ( 得分 = ( 当前重量 - 最小方案的重量) / (当前成本-最小方案的成))
     * @input maps:所有路径信息
     * @Return 最优的一条路径
     */
    ...
    public Map<String, Object> findTwoPointInfo(Map<String, String> twoMap,
        ...
        if (adjacencyMatrixGraph.getAllPoint().indexOf(startName) == -1
                || adjacencyMatrixGraph.getAllPoint().indexOf(endName) == -1) {
            return null;
        }

        // 边，用电器位置点名称（起点和终点），返回最短路径索引
        // 回路绕线也可以直接调用这个方法，传入的值为所有分支打通情况下的值
        List<Integer> shortestPath = shortestPathSearch.findShortestPathBetweenTwoPoint(adjacencyMatrixGraph.getAdj(),
                adjacencyMatrixGraph.getAllPoint().indexOf(startName),
        ...
        if (shortestPath != null) {
            ...
            for (List<Integer> list : allPathBetweenTwoPoint) {
                ...
                sinaglePath.put("湿区两端连接器成本补偿", Double.parseDouble(
                        df.format(Double.parseDouble(materialsMsg.get("湿区成本补偿——连接器塑壳（元/端）")) * wetNumber)));
                sinaglePath.put("湿区两端防水塞成本补偿", Double
                        .parseDouble(df.format(Double.parseDouble(materialsMsg.get("湿区成本补偿——防水赛（元/个）")) * wetNumber)));
                ...
                if (keyExistsIgnoreCase(elecBusinessPrice, start) || keyExistsIgnoreCase(elecBusinessPrice, end)) {
                    if (keyExistsIgnoreCase(elecBusinessPrice, start)) {
                        twoPointMsg.put("回路导线成本",
                                (Double) twoPointMsg.get("回路导线成本") + getValueIgnoreCase(elecBusinessPrice, start));
                    } else {
                        twoPointMsg.put("回路导线成本",
                                (Double) twoPointMsg.get("回路导线成本") + getValueIgnoreCase(elecBusinessPrice, end));
                    }
                }
                sinaglePath.put("回路总成本",
                        Double.parseDouble(df.format(Double.parseDouble(sinaglePath.get("湿区两端连接器成本补偿").toString())
                ...
                sinaglePath.put("回路湿区成本加成",
                        Double.parseDouble(df.format(Double.parseDouble(sinaglePath.get("湿区两端连接器成本补偿").toString())
                                ...
                                + Double.parseDouble(twoPointMsg.get("inline湿区防水塞成本补偿").toString())
                                + Double.parseDouble(twoPointMsg.get("inline湿区连接器成本补偿").toString()))));
                sinaglePath.put("inline湿区连接器成本补偿", Double
                ...
                sinaglePath.put("回路导线成本",
                        Double.parseDouble(df.format(Double.parseDouble(twoPointMsg.get("回路导线成本").toString()))));
                sinaglePath.put("回路重量",
                        Double.parseDouble(df.format(Double.parseDouble(twoPointMsg.get("回路重量").toString()))));
                sinaglePath.put("回路长度",
                ...
                sinaglePath.put("回路理论直径",
                        Double.parseDouble(df.format(Double.parseDouble(twoPointMsg.get("外径").toString()))));
        ...
        // 湿区防水塞成本
        Double waterPrice = Double.parseDouble(bestMap.get("inline湿区防水塞成本补偿").toString())
                + Double.parseDouble(bestMap.get("湿区两端防水塞成本补偿").toString());
        // 湿区连接器塑壳成本
     ...
     * @input: elecBusinessPrice 读取导线选型的excel表的sheet2的信息
     * @Return: 当前族群中最优的一条信息
     *          {中心点名称=空调箱中点, 到1用电器的信息={最优路径={起点用电器名称=[FVCM-KL30],
     *          起点用电id=e3d0e010-3fbc-4af0-bab6-9ad8c7044a95, 起点位置名称=null,
     *          起点位置id=null, 终点用电器名称=ETCM-FVCM,
     *          终点用电id=b83a49d7-13ea-4633-9664-7bd32de3536a, 终点位置名称=前挡右出点,
     *          终点位置id=b2593b81-b586-4f32-91a8-b01df6ac3ae1, 回路属性=null, 导线选型=FLRY-B
     *          0.35, 回路编号=5353, 方案号=85342e86-dd12-498d-b6a4-a45a3bd96fce,
     *          所属系统=系统Others, 回路起点用电器接口编号=, 回路终点用电器接口编号=, 回路信号名=FVCM-KL30,
     *          湿区两端连接器成本补偿=0.0, 湿区两端防水塞成本补偿=0.0, 焊点名称=[FVCM-KL30],
     *          回路id=2e215948-968f-4680-8cd6-a59521e89736, 焊点位置名称=空调箱中点,
     *          焊点位置id=e3d0e010-3fbc-4af0-bab6-9ad8c7044a95, 回路途径分支点=[空调箱中点, 空调箱左点,
     *          仪表线左中点, 仪表线右中点, 仪表右侧点, 仪表线右侧顶棚inline点, 顶棚右前inline点, 顶棚右前点, 前挡右出点],
     *          回路总成本=8.793069999999998, 回路湿区成本加成=0.0, inline湿区连接器成本补偿=0.0,
     *          inline湿区防水塞成本补偿=0.0, 回路打断成本=0.0, 回路导线成本=8.593069999999999,
     *          回路总重量=0.7513066666666667, 回路总长度=1.4087, 回路打断分支=[], 回路打断次数=0,
     *          回路所有分支=[0fc9f2f4-0422-4429-b74f-0becb211b5da,
     *          8e0bcb1b-760b-40d3-ae03-2d305acc7313,
     *          6f3ecce5-c586-4b6f-a5af-51fb9ef7f0f5,
     *          f4171731-680e-40f7-8a91-75ac88f86514,
     *          23bd3efd-7651-44d5-a01d-1860ca3a8af2,
     *          9bb0d48f-3b62-44ed-bf10-6c52e9501e48,
     *          8d5fa636-f386-4194-b559-8cfa944d6350,
     *          e55233dd-3a8d-4815-adf7-620e39687589], 回路所有分支数量=8, 回路直径=2.1},
     *          起点=空调箱中点, 终点=前挡右出点}, 到2用电器的信息={最优路径={起点用电器名称=[FVCM-KL30],
     *          起点用电id=e3d0e010-3fbc-4af0-bab6-9ad8c7044a95, 起点位置名称=null,
     *          起点位置id=null, 终点用电器名称=FVCM-A40D,
     *          终点用电id=3b6ee76d-6a32-4714-9c87-0cbc5dde65b5, 终点位置名称=空调箱右点,
     *          终点位置id=d6016448-b2a9-45a1-873b-22c7d6999972, 回路属性=null, 导线选型=FLRY-B
     *          0.35, 回路编号=5354, 方案号=85342e86-dd12-498d-b6a4-a45a3bd96fce,
     *          所属系统=系统Others, 回路起点用电器接口编号=, 回路终点用电器接口编号=, 回路信号名=FVCM-KL30,
     *          湿区两端连接器成本补偿=0.0, 湿区两端防水塞成本补偿=0.0, 焊点名称=[FVCM-KL30],
     *          回路id=661ebebb-4ce2-46a5-bfd6-1564a6f48534, 焊点位置名称=空调箱中点,
     *          焊点位置id=e3d0e010-3fbc-4af0-bab6-9ad8c7044a95, 回路途径分支点=[空调箱中点, 空调箱右点],
     *          回路总成本=0.2, 回路湿区成本加成=0.0, inline湿区连接器成本补偿=0.0, inline湿区防水塞成本补偿=0.0,
     *          回路打断成本=0.0, 回路导线成本=0.0, 回路总重量=0.0, 回路总长度=0.0, 回路打断分支=[], 回路打断次数=0,
     *          回路所有分支=[], 回路所有分支数量=1, 回路直径=2.1}, 起点=空调箱中点, 终点=空调箱右点},
     *          到3用电器的信息={最优路径={起点用电器名称=[FVCM-KL30],
     *          起点用电id=e3d0e010-3fbc-4af0-bab6-9ad8c7044a95, 起点位置名称=null,
     *          起点位置id=null, 终点用电器名称=UEC,
     *          终点用电id=f82321cc-3ae6-4c0f-9ac9-04bad98e120b, 终点位置名称=前舱左纵梁中点,
     *          终点位置id=a8e70607-a4a6-44cd-8708-7eca48590228, 回路属性=二级配电-KL.30,
     *          导线选型=FLRY-B 0.35, 回路编号=5355,
     *          方案号=85342e86-dd12-498d-b6a4-a45a3bd96fce, 所属系统=系统Others,
     *          回路起点用电器接口编号=, 回路终点用电器接口编号=J2, 回路信号名=FVCM-KL30, 湿区两端连接器成本补偿=0.0,
     *          湿区两端防水塞成本补偿=0.0, 焊点名称=[FVCM-KL30],
     *          回路id=ead061cf-254f-4cc9-9bef-953e5c5b6049, 焊点位置名称=空调箱中点,
     *          焊点位置id=e3d0e010-3fbc-4af0-bab6-9ad8c7044a95, 回路途径分支点=[空调箱中点, 空调箱左点,
     *          仪表线左中点, 仪表左侧点, 仪表线左侧顶棚inline点, 顶棚左前inline点, 车身线左前inline点,
     *          前舱线左后inline点, 前舱左纵梁后点, 前舱左纵梁后中点, 前舱左纵梁中点], 回路总成本=10.71388,
     *          回路湿区成本加成=0.0, inline湿区连接器成本补偿=0.0, inline湿区防水塞成本补偿=0.0, 回路打断成本=0.2,
     *          回路导线成本=10.31388, 回路总重量=0.9017599999999999, 回路总长度=1.6907999999999999,
     *          回路打断分支=[d89d0217-8ef0-41f2-835c-49ca8f7e069c], 回路打断次数=1,
     *          回路所有分支=[0fc9f2f4-0422-4429-b74f-0becb211b5da,
     *          8e0bcb1b-760b-40d3-ae03-2d305acc7313,
     *          a57758e1-fd04-416c-b577-7aba48c11e0f,
     *          4c57dd24-da2b-44f4-b63f-e9ad7ac90163,
     *          893aea5f-74a4-4f28-a2c8-e54d688d5b41,
     *          322031f6-af42-4829-9227-4eb50eadc1a5,
     *          d89d0217-8ef0-41f2-835c-49ca8f7e069c,
     *          8a464b8f-d481-465a-a7ce-f320d548d83e,
     *          4a1dc59b-e378-4bd9-a535-db19647faae7,
     *          6ef1cf41-e4f4-404b-a93a-c3f015b42608], 回路所有分支数量=10, 回路直径=2.1},
     *          起点=空调箱中点, 终点=前舱左纵梁中点}}
     */
    public Map<String, Object> findGroupInfo(String name,
        ...
        List<String> centerPoint = findCenterPoint(adjacencyMatrixGraph.getAdj(), adjacencyMatrixGraph.getAllPoint(),
                appNode.stream().collect(Collectors.toList()));
        if (centerPoint == null) {
            return null;
        }
        // 单个中心点到到各个点信息
        ...
        if (centerPoint.size() > 0) {
            for (String integer : centerPoint) {
                ...
                for (Map<String, String> stringMap : multiLoopinfoMap) {
                    ...
                    List<List<Integer>> allPathBetweenTwoPoint = findAllPath.findAllPathBetweenTwoPoint(
                            adjacencyMatrixGraph.getAdj(), adjacencyMatrixGraph.getAllPoint().indexOf(integer),
                            adjacencyMatrixGraph.getAllPoint().indexOf(endName));
                    ...
                    for (List<Integer> list : allPathBetweenTwoPoint) {
                        ...
                        CalculateCircuitInfo acceptLoopInfo = new CalculateCircuitInfo();
                        List<String> listname = convertPathToNumbers(list, adjacencyMatrixGraph.getAllPoint());
                        Map<String, Object> twoPointMsg = acceptLoopInfo.calculateCircuitInfo(materials, listname,
                                projectInfo, elecFixedLocationLibrary);
                        twoPointMsg.put("端子成本", Double.parseDouble(materialsMsg.get("导线打断成本（元/次）")) / 2);// 端子成本
                                                                                                         // 实际上指的是导线打断成本
                        ...
                        sinaglePath.put("湿区两端连接器成本补偿", Double.parseDouble(
                                df.format(Double.parseDouble(materialsMsg.get("湿区成本补偿——连接器塑壳（元/端）")) * wetNumber)));
                        sinaglePath.put("湿区两端防水塞成本补偿", Double.parseDouble(
                        ...
                        sinaglePath.put("回路途径分支点", listname);
                        if (keyExistsIgnoreCase(elecBusinessPrice, start)
                                || keyExistsIgnoreCase(elecBusinessPrice, end)) {
                            if (keyExistsIgnoreCase(elecBusinessPrice, start)) {
                                twoPointMsg.put("回路导线成本", (Double) twoPointMsg.get("回路导线成本")
                                        + getValueIgnoreCase(elecBusinessPrice, start));
                            } else {
                                twoPointMsg.put("回路导线成本", (Double) twoPointMsg.get("回路导线成本")
                                        + getValueIgnoreCase(elecBusinessPrice, end));
                            }
                        }
                        sinaglePath.put("回路总成本",
                                Double.parseDouble(
                                        df.format(Double.parseDouble(sinaglePath.get("湿区两端连接器成本补偿").toString())
                                                + Double.parseDouble(sinaglePath.get("湿区两端防水塞成本补偿").toString())
                                                + Double.parseDouble(twoPointMsg.get("inline湿区防水塞成本补偿").toString())
                                                + Double.parseDouble(twoPointMsg.get("inline湿区连接器成本补偿").toString())
                                                + Double.parseDouble(twoPointMsg.get("回路打断成本").toString())
                        ...
                        sinaglePath.put("回路湿区成本加成",
                                Double.parseDouble(
                                        df.format(Double.parseDouble(sinaglePath.get("湿区两端连接器成本补偿").toString())
                                                + Double.parseDouble(sinaglePath.get("湿区两端防水塞成本补偿").toString())
                                                + (Double) twoPointMsg.get("inline湿区防水塞成本补偿")
                                                + Double.parseDouble(twoPointMsg.get("inline湿区连接器成本补偿").toString()))));
                        sinaglePath.put("inline湿区连接器成本补偿", Double.parseDouble(
                        ...
                        sinaglePath.put("回路导线成本", Double
                                .parseDouble(df.format(Double.parseDouble(twoPointMsg.get("回路导线成本").toString()))));
                        sinaglePath.put("回路重量",
                                Double.parseDouble(df.format(Double.parseDouble(twoPointMsg.get("回路重量").toString()))));
                        sinaglePath.put("回路长度",
                                Double.parseDouble(df.format(Double.parseDouble(twoPointMsg.get("回路长度").toString()))));
                    ...
                    // 湿区防水塞成本
                    Double waterPrice = Double.parseDouble(bestPath.get("inline湿区防水塞成本补偿").toString())
                            + Double.parseDouble(bestPath.get("湿区两端防水塞成本补偿").toString());
                    // 湿区连接器塑壳成本
            ...
            for (Map<String, String> stringMap : multiLoopinfoMap) {
                ...
                List<String> list = new ArrayList<>();
                list.add("");
                Map<String, Object> twoPointMsg = acceptLoopInfo.calculateCircuitInfo(materials, list, projectInfo,
                        elecFixedLocationLibrary);
                ...
                sinaglePath.put("终点位置名称", stringMap.get("回路终点用电器").startsWith("[") ? null : endName);
                sinaglePath.put("终点位置id",
                        stringMap.get("回路终点用电器").startsWith("[") ? null : findIdByName(endName, pointList));
                sinaglePath.put("回路属性", stringMap.get("回路属性"));
                sinaglePath.put("导线选型", stringMap.get("回路导线选型"));
                sinaglePath.put("回路编号", stringMap.get("回路编号"));
                sinaglePath.put("方案号", stringMap.get("方案号"));
                ...
                // 湿区防水塞成本
                Double waterPrice = Double.parseDouble(objectMap1.get("inline湿区防水塞成本补偿").toString())
                        + Double.parseDouble(objectMap1.get("湿区两端防水塞成本补偿").toString());
                // 湿区连接器塑壳成本
    ...
    /**
     * @Description 获取最优的一条路径(再次遍历所有方案 ， 根据与最小成本方案的成本和重量差异计算得分 ， 最小成本方案得分为 - 4.0 (
     *              得分 = ( 当前重量 - 最小方案的重量) / (当前成本-最小方案的成))
     * @input maps:所有路径信息
     * @Return 最优的一条路径
     */
    ...
    /**
     * @Description 找出最优的一条中心点（首先找出每个中心点中的 回路总成本、回路总量、回路长度的最大值和最小值 然后按照
     ...
     * @Return 返回中心点的名称 [仪表线左中点, 前顶横梁左中点, 前舱右纵梁中点, 前舱中部右点, 空调箱中点, 仪表线左侧inline点,
     *         仪表线左侧顶棚inline点, 仪表线右侧inline点, 前舱线左后inline点, 前挡右出点, 前舱中后部左点, 前舱左纵梁后点,
     *         车身线右前inline点, 仪表右侧点, 前舱中后部中点, 前舱中后部右点, 空调箱左点, 前舱左纵梁后中点, 前围板内右点,
     *         前顶横梁右中点, 顶棚右前点, 前围板内中点, 仪表线右中点, 顶棚右前inline点, 仪表线右侧顶棚inline点, 前顶横梁左点,
     *         空调箱右点, 前舱右纵梁后点, 车身线左前inline点, 顶棚左前点, 前舱线右后inline点]
     */
    public List<String> findCenterPoint(List<List<Integer>> adj, List<String> allPoint, List<String> terminal) {
        ...
        for (int i = 0; i < terminal.size(); i++) {
            for (int j = i + 1; j < terminal.size(); j++) {
                ...
                List<Integer> shortestPathBetweenTwoPoint = findShortestPath.findShortestPathBetweenTwoPoint(adj, start,
                        endpoint);
