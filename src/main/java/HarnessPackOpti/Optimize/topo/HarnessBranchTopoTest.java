...
//                List<Map<String, Object>> mapList = handleAndShowTop(jsonMap, "abnormal");
...
//        对当前的平均数做一个排序找到第一个闭合平均值小于6的B的数量
...
//            }
//        });
        Integer minLoopNumber = 18;
        Integer maxLoopNumber = 27;
...
//        接下来就是根据上面找到的key进行一个样本的创建
        List<List<String>> simpleList = initialOptimize(minLoopNumber, maxLoopNumber, initialScheme, togetherBCList, conformList, normList, onlyNameS, edges, appPositions, eleclection, mutexMap,
                mutexGroupList, chooseOneList, initialCanchangeBList, sortedMapExcel, sortedMap, circuitInfoList);
        simpleList.add(primeList);
        if (optimizeStopStatusStore.get(optimizeRecordId) == false) {
            if (TopDetail != null && TopDetail.size() > 4) {
                TopDetail.subList(4, TopDetail.size()).clear();
            }
            initializeCaseResultMap.put("finishStatue", "abnormal");
            List<Map<String, Object>> mapList = handleAndShowTop(jsonMap, "abnormal");
            mapList.add(initializeCaseResultMap);
...
////            只有当迭代的结果top1都是同一个值的时候    才结束迭代
//            findBest = hybridization(findBest, onlyNameS, normList, conformList, togetherBCList, canChangeS, edges, appPositions, eleclection,
//                    mutexMap, minLoopNumber, maxLoopNumber, initialScheme, wearId, jsonMap, edgeChooseBS, chooseOneList, mutexGroupList,
//                    initialCanchangeBList, sortedMapExcel, sortedMap, circuitInfoList);
//            TopDetail = findBest;
//            if (optimizeStopStatusStore.get(optimizeRecordId) == false) {
//                if (TopDetail != null && TopDetail.size() > 4) {
//                    TopDetail.subList(4, TopDetail.size()).clear();
...
//                return objectMapper.writeValueAsString(mapList);
//            }
//            writerToCSV.writeToExcel(findBest, Integer.toString(hybridizationNumber));
...
//            hybridizationNumber++;
//        }
        findBest = bestOptionVariation(findBest, singleBCList, singleSCList, singleBSList, singleBSCList, normList, jsonMap, eleclection, wearId, mutexMap, chooseOneList, togetherBCList);
        TopDetail = findBest;
        List<Map<String, Object>> mapList = handleAndShowTop(jsonMap, "normal");
    ...
    /**
     * @Description: 对top的方案进行再次变异   结果中分支打断状况为S的 可以变化的进行变化  在总成本小于3的情况下  选择当前的方案
     * @input: findBest  当前最优的top10 方案
     * @input: initialCanchangeSToBList   分支可以从S转为B的集合
...
//            对每一个S 还原成C     检查是否存在闭环  如果存在闭环  在闭环里面能够变为B的分支  检查当前的方案是否合理   合理的情况下计算成本   如果成本与原方案相比小于3  则用该方案
            System.out.println(canRestoreid.size() + "=======");
            int demo = 0;
...
//            对最终的方案进行一个计算  并且按照格式进行一个返回
    ...
    /**
     * @Description: 对top10的数据进行一个处理返回给前端
     * @input:
     * @inputExample:
     * @Return:
     */
    public List<Map<String, Object>> handleAndShowTop(Map<String, Object> jsonMap, String finishStatue) throws Exception {
        ...
        if (TopDetail != null) {
            for (Map<String, Object> objectMap : TopDetail) {
     ...
     * @input edgeChooseBS
     * @input chooseOneList
     * @input mutexGroupList
     * @input initialCanchangeBList
     * @input sortedMapExcel
     * @input sortedMap
     * @input circuitInfoList
     * @input breakCostMap  每个分支打断的成本集合
     * @Return
     */
    public List<Map<String, Object>> hybridization(List<Map<String, Object>> findBest,
                                                   ...
                                                   List<String> initialCanchangeBList,
...
//        首先将给定的top10的方案进行一个还原   将里面符合要求的S改为C
...
//        将之前的top10的方案也添加到新的容器里面
        simple.addAll(topTenList);
        List<List<String>> changebTOc = new ArrayList<>(topTenList);
        List<List<String>> changecTOb = new ArrayList<>(topTenList);
...
//        将simple添加到仓库里面
        ...
        while (simple.size() < HybridizationLessRandomSamleNumber) {
            List<List<String>> simpleList = initialOptimize(minLoopNumber, maxLoopNumber, initialScheme, togetherBCList, conformList, normList, onlyNameS, edges, appPositions, eleclection, mutexMap, mutexGroupList, chooseOneList, initialCanchangeBList, sortedMapExcel, sortedMap, circuitInfoList);
...
//        接下来就是对simple 进行一个分支闭环的检查
    ...
    public List<Map<String, Object>> changeAndFindBest(List<List<String>> simpleList,
        ...
        //       serviceableMap  有用的分支打断情况
        List<List<String>> serviceableList = new ArrayList<>();
...
//        每个方案进行计算
        List<Map<String, Object>> topBeat = findBest(resultList, "成本");
        return topBeat;
    }
...
//               组团变化每一个组根据随机生成的数字惊醒选择B 还是C   0为B  1为C
                ...
                List<String> getTopintersection = percentage.get("getLastintersection");
                getTopintersection.retainAll(initialCanchangeBList);

                List<String> list = selectId(conformList, i - needChangeBId.size());
                needChangeBId.addAll(list);
                List<String> changeList = initialScheme.stream().collect(Collectors.toList());
...
//               判断生成的list  是否在仓库里界面    再判断是否在resultList集合里面
                if (sonSate) {
    ...
    /**
     * @Description: List<Stringid 在Map < String, Double> breakCostMap中每一个id作为一个key对应的 double最小的一个key
     * @input: ids  id集合
