...
public class GenerateTopoMatrix {
    ...
    // 获取邻接矩阵基本信息
    public GenerateTopoMatrix(List<String> startPointName, List<String> endPointName,
            List<List<String>> branchBreakList) {
        ...
        // 预先建 point→index 表，O(1) 查表替代 O(V) 的 equals 循环，整体 O(E·V) → O(E)
        Map<String, Integer> pointToIdx = new HashMap<>(this.V * 2);
        for (int i = 0; i < this.V; i++) {
            pointToIdx.put(this.allPoint.get(i), i);
        }
        List<Integer> startPointRowNumber = new ArrayList<>(this.startPoint.size());
        for (String k : this.startPoint) {
            startPointRowNumber.add(pointToIdx.get(k));
        }
        List<Integer> endPointColumnNumber = new ArrayList<>(this.endPoint.size());
        for (String k : this.endPoint) {
            endPointColumnNumber.add(pointToIdx.get(k));
        }
        this.startPointRowNumber = startPointRowNumber;
        this.endPointColumnNumber = endPointColumnNumber;
        // System.out.println("startPointRowNumber个数"+startPointRowNumber.size()+":
        // "+startPointRowNumber);
        // System.out.println("endPointColumnNumber个数"+endPointColumnNumber.size()+":
        // "+endPointColumnNumber);
    }
