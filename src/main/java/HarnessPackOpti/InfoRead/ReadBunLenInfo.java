...
public class ReadBunLenInfo {
   public Map<String,Object> getBunLenInfo(Map<String,Object> mapFromProject){
        /*
        需要读取的参数信息有：
        ...
        -来自map中的sizeInfoList列表：所有关键尺寸信息  关键尺寸id、关键尺寸朝向toward、关键尺寸名称criticalDimension、
                                                  关键尺寸说明criticalDimensionName、关键尺寸长度size
        -来自map中的projectInfo列表：项目基本信息    基本信息id、项目名称project、左右驾信息drivingType、车辆类型bodyStyle、
                                                动力类型powerType
         */

        // 读取外部输入Map中的topoInfo、points列表、edges列表信息
        // 读取外部输入Map中的sizeInfoMap列表信息、projectInfoMap信息
        ...
        List<Map<String, Object>> sizeInfoMap = (List<Map<String, Object>>) mapFromProject.get("sizeInfoList");
...
//        System.out.println("sizeInfoMap:"+sizeInfoMap);
//        System.out.println("projectInfoMap:"+projectInfoMap);
       ...
       List<Map<String, Object>> edges = new ArrayList<>();//中层的map列表，存放所有分支的信息
        List<Map<String, Object>> sizeInfo = new ArrayList<>();//中层的map列表，存放所有关键尺寸信息
        Map<String, Object>  projectInfo = new HashMap<>();//中层的map列表，存放项目基本信息
...
//       for (Map<String, Object> k:sizeInfoMap){
//           Map<String, Object> size = new HashMap<>();//内层的map，存放单个关键尺寸的信息
//           size.put("关键尺寸id编号",k.get("id"));
//           size.put("关键尺寸朝向",k.get("toward"));
//           size.put("关键尺寸名称",k.get("criticalDimension"));
//           size.put("关键尺寸说明",k.get("criticalDimensionName"));
//           size.put("关键尺寸长度",k.get("size"));
//           sizeInfo.add(size);//每个关键尺寸的信息放入列表
//       }
